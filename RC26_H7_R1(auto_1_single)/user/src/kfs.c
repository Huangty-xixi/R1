...
void kfs_catch_mode()//¼Ð×¦¿ØÖÆº¯Êý
{
	...
	else if(RCctrl.kfs_catch ==1)  //°´Å¥´¥·¢¼Ð×¦
  {
		if(kfs_catch.position > 1.45f)
   {
	 ...
	 else if(kfs_catch.position < 1.45f && kfs_catch.position > 0.35f)
   {
	   k_catch[0] = 0.0f;
		 k_catch[1] = -(((float)kfs_catch.position -0.35f)/(1.10f)*4.0f + 1.0f);
		 k_catch[2] = 0.0f;
		 k_catch[3] = 1.0f/(-k_catch[1]);
		 k_catch[4] = 0.0f;
	 }
	 else if(kfs_catch.position < 0.35f)
   {
	   k_catch[0] = 0.0f;
		 k_catch[1] = 0.0f;
	   k_catch[2] = 0.0f;
