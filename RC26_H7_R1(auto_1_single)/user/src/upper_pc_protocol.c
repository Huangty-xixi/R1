 ...
 * @brief   R2 雷达协议解析（在 STM32 上实现，此文件用于上位机协议）
 *
 * 使用方式:
 *   1. rc_init(uart1_putc, HAL_GetTick)，ODOM 帧写入内部 latest_odom，可通过 rc_get_latest_odom / rc_odom_is_valid 访问
 *   2. 如需 ODOM 回调，在 rc_init 后调用 rc_set_odom_callback(cb)，未注册回调时仍会更新 latest_odom
 *   3. UART RX 中断中喂入字节: rc_feed_byte(rx_data);
 *   4. 主循环: rc_poll();
 */
...
/* 如果未定义 APP_ZONE2_RED_SIDE，则默认使用蓝方配置 */
...
/* 接收缓冲区 */
 uint8_t  rx_buf[RC_FRAME_MAX_SIZE];
static uint16_t rx_idx = 0;
static uint8_t  rx_sync = 0;  /* 0=未同步 1=找到 SYNC1 2=找到 SYNC2 */
...
void rc_feed_byte(uint8_t byte)
{
    ...
    case 1:
        ...
        break;
    case 2:
        if (rx_idx >= RC_FRAME_MAX_SIZE) { rx_sync = 0; rx_idx = 0; break; }
        rx_buf[rx_idx++] = byte;
        if (rx_idx >= RC_FRAME_HEADER_SIZE) {
            uint8_t  cmd = rx_buf[2];
            uint16_t len = (uint16_t)rx_buf[3] | ((uint16_t)rx_buf[4] << 8);
            if (len > RC_FRAME_MAX_PAYLOAD) { rx_sync = 0; rx_idx = 0; break; }
...
#define RC_REQ_INTERVAL_MS 5000U
volatile uint32_t rc_req_count = 0;

void rc_poll(void)
{
    if (!get_ms) return;
    uint32_t now = get_ms();
    if ((odom_last_ms == 0 || !rc_odom_is_valid()) && (now - last_req_ms >= RC_REQ_INTERVAL_MS))
    {
        rc_request_odom();
        rc_req_count++;
        last_req_ms = now;
...
void rc_request_odom(void)
{
    send_frame(RC_CMD_REQUEST_ODOM, NULL, 0);
}
