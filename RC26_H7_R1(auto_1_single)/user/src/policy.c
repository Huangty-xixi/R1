...
int path[20];                // �洢·�����У����ӱ�ţ�
int path_len;                // ·������
int picked_k2[4];            // ��ʰȡ��K2λ��
int picked_cnt;              // ʰȡ��K2����
int removed_k1;              // ���Ƴ���K1λ��
int removed_k1_2;            // �Ƴ�2��K1ʱ�ĵڶ���λ��
int removed_cnt;             // ʵ���Ƴ��˼���K1��1��2��
int target_k2;               // Ŀ��ʰȡK2����
...
static int s_bottom_k2_mask; /* �ײ���(1/2/3�Ÿ�)K2��bitmask��bfs_run�ڸ�ֵ */
...
/* ================================================================
   solve_best: 6-level priority cascade with step arbitration
     All levels capped at MAX_STEPS; on overflow, fallback to next.
     If no level fits, best-effort: return the shortest path found.

     Priority order (R2 prefers 3 K2, then 2, then 1 last resort):
       Level 1: 3 K2 + 1 K1  (ideal)
       Level 2: 3 K2 + 2 K1
       Level 3: 2 K2 + 1 K1
       Level 4: 2 K2 + 2 K1
       Level 5: 1 K2 + 1 K1  (last resort)
       Level 6: 1 K2 + 2 K1
   ================================================================ */
#define POLICY_MAX_STEPS  6
                       ...
                       int *out_removed_k1, int *out_target_k2)
{
    ...
    /* cascade table: {target_k2, remove_n} in priority order */
    static const struct { int tk; int rn; } levels[] = {
        {3, 1},  /* Level 1: 3 K2 + 1 K1 — most preferred */
        {3, 2},  /* Level 2: 3 K2 + 2 K1 */
        {2, 1},  /* Level 3: 2 K2 + 1 K1 */
        {2, 2},  /* Level 4: 2 K2 + 2 K1 */
        {1, 1},  /* Level 5: 1 K2 + 1 K1 — last resort */
        {1, 2},  /* Level 6: 1 K2 + 2 K1 */
    };
    const int num_levels = sizeof(levels) / sizeof(levels[0]);

    /* best-effort fallback: record the shortest result across all levels */
    int  best_path[20], best_pl = 999, best_pk[4], best_pc = 0;
    int  best_rk1 = 0, best_rk2 = 0, best_tk = 0, best_rn = 0;

    for (int i = 0; i < num_levels; i++) {
        int tk = levels[i].tk;
        int rn = levels[i].rn;

        if (policy_solve_n(tk, rn, path, &pl, pk, &pc, &rk1, &rk2)) {
            /* track shortest path as best-effort fallback */
            if (pl < best_pl) {
                best_pl = pl;
                memcpy(best_path, path, pl * sizeof(int));
                best_pc = pc;
                memcpy(best_pk, pk, pc * sizeof(int));
                best_rk1 = rk1;
                best_rk2 = rk2;
                best_tk = tk;
                best_rn = rn;
            ...
            /* accept immediately if within step budget */
            if ((pl - 1) <= POLICY_MAX_STEPS) {
                *out_target_k2 = tk;
                removed_cnt = rn;
                goto output;
            }
    ...
    /* no level fits the step cap — best-effort fallback */
    if (best_pl < 999) {
        pl = best_pl;
        memcpy(path, best_path, best_pl * sizeof(int));
        pc = best_pc;
        memcpy(pk, best_pk, best_pc * sizeof(int));
        rk1 = best_rk1;
        rk2 = best_rk2;
        *out_target_k2 = best_tk;
        removed_cnt = best_rn;
        goto output;
    }
...
int main(void)
{
    ...
    /* Step-by-step: trace all 6 cascade levels */
    {
        int path[20], pl, pk[4], pc, rk1, rk2;
        static const struct { int tk; int rn; const char *desc; } levels[] = {
            {3, 1, "Level 1: 3 K2 + 1 K1"},
            {3, 2, "Level 2: 3 K2 + 2 K1"},
            {2, 1, "Level 3: 2 K2 + 1 K1"},
            {2, 2, "Level 4: 2 K2 + 2 K1"},
            {1, 1, "Level 5: 1 K2 + 1 K1"},
            {1, 2, "Level 6: 1 K2 + 2 K1"},
        };
        int n = sizeof(levels) / sizeof(levels[0]);

        for (int i = 0; i < n; i++) {
            printf("=== %s ===", levels[i].desc);
            if (policy_solve_n(levels[i].tk, levels[i].rn,
                               path, &pl, pk, &pc, &rk1, &rk2)) {
                printf("  found, steps=%d", pl - 1);
                if ((pl - 1) <= POLICY_MAX_STEPS) {
                    print_result(" ACCEPT", levels[i].tk,
                                 rk1, rk2, levels[i].rn, path, pl, pk, pc);
                } else {
                    printf("  -- REJECTED (>%d steps)\n", POLICY_MAX_STEPS);
                }
            } else {
                printf("  -- no solution\n");
            }
    ...
    /* policy_solve_best() integration test */
    {
        int path[20], pl, pk[4], pc, rk, tk;
        printf("\n=== policy_solve_best() ===\n");
        if (policy_solve_best(path, &pl, pk, &pc, &rk, &tk)) {
            const char *tag = ((pl - 1) <= POLICY_MAX_STEPS) ? "BEST" : "BEST-EFFORT";
            print_result(tag, tk, rk, removed_k1_2, removed_cnt, path, pl, pk, pc);
        } else {
            printf("  -- No solution!\n");
        }
    }
